...
# Face mosaic function
def face_mosaic(image, x, y, w, h, ratio=0.1):
    """
    Applies a mosaic effect to a detected face in an image.
    :param image: Image where the face is detected
    :param x: X-coordinate of the face's bounding box
    :param y: Y-coordinate of the face's bounding box
    :param w: Width of the face's bounding box
    :param h: Height of the face's bounding box
            ...
            if det is not None and len(det):
                ...
                if len(outputs[i]) > 0:
                    for j, (output, conf) in enumerate(zip(outputs[i], confs)):
                        ...
                        # Apply mosaic only on faces if 'draw' is enabled
                        if draw and cls == 0:  # Assuming class 0 is 'person'
                            # Apply mosaic to the detected face
                            im0 = face_mosaic(im0, int(bbox_left), int(bbox_top),
                                            int(bbox_right - bbox_left), int(bbox_bottom - bbox_top))
